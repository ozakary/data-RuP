...
def get_cache_filename(temperatures, n_bins, value_range):
    ...
    return f"cache_trimer_{hash_obj.hexdigest()}.npz"
...
def process_single_temperature(args):
    ...
    csv_filename = os.path.join(output_dir, f'trimer_order_{temp}K.csv')
    
    # Load data efficiently
    values = []
    chunksize = 1000000
    for chunk in pd.read_csv(csv_filename, chunksize=chunksize, usecols=['trimer_order']):
        values.append(chunk['trimer_order'].values)
...
def load_or_compute_histograms(temperatures, output_dir='.', n_bins=100, 
    ...
    if value_range is None:
        ...
        csv_file = os.path.join(output_dir, f'trimer_order_{first_temp}K.csv')
        df_sample = pd.read_csv(csv_file, nrows=10000)
        value_range = (df_sample['trimer_order'].min() - 0.05, 
                      df_sample['trimer_order'].max() + 0.05)
...
def plot_histogram_with_kde(temp, values, kde, bin_edges, output_dir='.', 
                            value_range=(0.0, 0.3), figsize=(4.0, 4.0)):
    """
    ...
    # Plot histogram
    bin_edges_plot = 100#np.linspace(value_range[0], value_range[1], len(bin_edges))
    counts, _, patches = ax.hist(values, bins=bin_edges_plot, density=True, 
    ...
    # Labels and title
    ax.set_xlabel('Trimer Bond Order')
    ax.set_ylabel('Density')
#    ax.set_title(f'Trimer Bond Order Distribution at {temp} K')
    ...
    # Save figure
    fig_filename = os.path.join(output_dir, f'trimer_order_histogram_{temp}K.svg')
...
def plot_2d_heatmap(temperatures, output_dir='.', n_bins=100, 
    ...
    """
    Create 2D heatmap with trimer order on x-axis, temperature on y-axis.
    Overlays 1 population curve.
    ...
    show_population_curve : bool
        Whether to overlay dashed curve for population average
    curve_color : str
        Color for the population curve
    curve_linewidth : float
        Line width for population curve
    curve_alpha : float
        Alpha (transparency) for population curve
    show_error_band : bool
        Whether to show ±1 std deviation band around curve
    error_band_alpha : float
        Alpha for error band
    ...
    x_values = np.linspace(0.0, 0.3, value_resolution)#bin_edges[0], bin_edges[-1], value_resolution)
    ...
    # Overlay population curve
    if show_population_curve and pop_stats is not None:
        print("\nAdding population average curve...")
        
        temps_pop = pop_stats['temperature'].values
        mean_values = pop_stats['mean'].values
        std_values = pop_stats['std'].values
        
        # Plot mean curve
        ax.plot(mean_values, temps_pop, '--', color=curve_color, 
                linewidth=curve_linewidth, alpha=curve_alpha,
                label='Mean')
        
        # Optionally add error band
        if show_error_band:
            ax.fill_betweenx(temps_pop, mean_values - std_values, mean_values + std_values,
                            color=curve_color, alpha=error_band_alpha)
        
#        ax.legend(loc='upper left', framealpha=0.7, fontsize=9)
        
        print(f"  Mean at {temps_pop[0]:.0f}K: {mean_values[0]:.4f}")
        print(f"  Mean at {temps_pop[-1]:.0f}K: {mean_values[-1]:.4f}")
    ...
    # Set limits
    ax.set_xlim(0.0, 0.3)#bin_edges[0], bin_edges[-1])
    ax.set_ylim(temp_array.min(), temp_array.max())
    
    plt.tight_layout()
    
    # Save figure
    fig_filename = os.path.join(output_dir, 'trimer_order_2d_heatmap.png')
    plt.savefig(fig_filename, dpi=300, bbox_inches='tight')
    ...
    if plot_individual_histograms:
        ...
        for hist_temp in histogram_temps:
            if hist_temp in temperatures:
                ...
                csv_filename = os.path.join(output_dir, f'trimer_order_{hist_temp}K.csv')
                ...
                plot_histogram_with_kde(hist_temp, values, kde, bin_edges, 
                                       output_dir, value_range=(0.0, 0.3), 
...
def main():
    ...
    # Population curve settings (1 curve for trimer)
    show_population_curve = True
    curve_color = 'k'
    curve_linewidth = 2.2
    curve_alpha = 1.0
    ...
    histogram_figsize = (4.0, 4.0)  # Figure size for histogram plots (independent of 2D heatmap)
    ...
    print(f"  - Number of populations: 1")
    ...
    print("\nGenerated files:")
    print("  1. trimer_order_2d_heatmap.png (2D heatmap with horizontal lines)")
    if plot_individual_histograms:
        for hist_temp in histogram_temps:
            print(f"  {histogram_temps.index(hist_temp)+2}. trimer_order_histogram_{hist_temp}K.png")
