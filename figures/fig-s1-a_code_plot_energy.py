...
def plot_metrics(csv_file):
    ...
    ax.loglog(df['epoch'], df['training_loss_e'], '-', color='#264653', label='$E_{Loss}^{T}$')   
    ax.loglog(df['epoch'], df['training_e_mae'], '-', color='#2A9D8F', label='$E_{MAE}^{T}$')
    ax.loglog(df['epoch'], df['training_e_rmse'], '-', color='#E76F51', label='$E_{RMSE}^{T}$')
    ax.loglog(df['epoch'], df['validation_loss_e'], '--', color='#264653', label='$E_{Loss}^{V}$')   
    ax.loglog(df['epoch'], df['validation_e_mae'], '--', color='#2A9D8F', label='$E_{MAE}^{V}$')
    ax.loglog(df['epoch'], df['validation_e_rmse'], '--', color='#E76F51', label='$E_{RMSE}^{V}$') 
    ax.loglog(df['epoch'], df['LR']*0.00001, '-', color='#E9C46A', label=r'$LR \times 10^{-9}$') 
    ...
    # Show the plot
    plt.tight_layout()
    plt.savefig("plot_ml_process_energy.svg", bbox_inches='tight')
    plt.show()
