    ...
    with open(log_file, 'r') as f:
        for line in f:
            try:
                ...
                # Extract learning rate if available (you may need to adjust based on your log format)
...
def plot_energy_metrics(log_file, output_file='plot_ml_process_energy.svg'):
    """Plot energy-related metrics from training log."""
    ...
    val_mae = [d.get('mae_e_per_atom', np.nan) for d in validation_agg]
    val_rmse = [d.get('rmse_e_per_atom', np.nan) for d in validation_agg]
    
    # Create the plot
    plt.figure(figsize=(10, 5))
    
    # Plot training loss
    plt.loglog(train_epochs, train_loss, '-', color='#264653', label=r'$E_{\text{Loss}}^{\text{T}}$')
    
    # Plot validation metrics
    if any(~np.isnan(val_loss)):
        plt.loglog(val_epochs, val_loss, '--', color='#264653', label=r'$E_{\text{Loss}}^{\text{V}}$')
    if any(~np.isnan(val_mae)):
        plt.loglog(val_epochs, val_mae, '--', color='#2A9D8F', label=r'$E_{\text{MAE}}^{\text{V}}$')
    if any(~np.isnan(val_rmse)):
        plt.loglog(val_epochs, val_rmse, '--', color='#E76F51', label=r'$E_{\text{RMSE}}^{\text{V}}$')
    
    # Add learning rate if available
    if lr_data:
        lr_agg = aggregate_by_epoch(lr_data)
        lr_epochs = [d['epoch'] for d in lr_agg]
        lr_values = [d['lr'] * 1e-9 for d in lr_agg]
        plt.loglog(lr_epochs, lr_values, '-', color='#E9C46A', label=r'$LR \times 10^{-9}$')
