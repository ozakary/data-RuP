...
def plot_metrics(csv_file):
    ...
    ax.loglog(df['epoch'], df['training_loss_stress'], '-', color='#264653', label=r'$\hat{s}_{Loss}^{T}$')   
    ax.loglog(df['epoch'], df['training_stress_mae'], '-', color='#2A9D8F', label=r'$\hat{s}_{MAE}^{T}$')
    ax.loglog(df['epoch'], df['training_stress_rmse'], '-', color='#E76F51', label=r'$\hat{s}_{RMSE}^{T}$')
    ax.loglog(df['epoch'], df['validation_loss_stress'], '--', color='#264653', label=r'$\hat{s}_{Loss}^{V}$')   
    ax.loglog(df['epoch'], df['validation_stress_mae'], '--', color='#2A9D8F', label=r'$\hat{s}_{MAE}^{V}$')
    ax.loglog(df['epoch'], df['validation_stress_rmse'], '--', color='#E76F51', label=r'$\hat{s}_{RMSE}^{V}$') 
    ax.loglog(df['epoch'], df['LR']*0.0001, '-', color='#E9C46A', label=r'$LR \times 10^{-6}$') 

    # Fix minor ticks for both axes
    ax.xaxis.set_minor_locator(ticker.LogLocator(base=10.0, subs=np.arange(2, 8) * 0.1, numticks=12))
    ax.yaxis.set_minor_locator(ticker.LogLocator(base=10.0, subs=np.arange(2, 5) * 0.1, numticks=12))
