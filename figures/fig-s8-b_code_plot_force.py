...
def plot_force_metrics(log_file, output_file='plot_ml_process_forces.svg'):
    """Plot force-related metrics from training log."""
    ...
    val_mae = [d.get('mae_f', np.nan) for d in validation_agg]
    val_rmse = [d.get('rmse_f', np.nan) for d in validation_agg]
    
    # Create the plot
    plt.figure(figsize=(10, 5))
    
    # Plot training loss
    plt.loglog(train_epochs, train_loss, '-', color='#264653', label=r'$\vec{f}_{\text{Loss}}^{\text{T}}$')
    
    # Plot validation metrics
    if any(~np.isnan(val_loss)):
        plt.loglog(val_epochs, val_loss, '--', color='#264653', label=r'$\vec{f}_{\text{Loss}}^{\text{V}}$')
    if any(~np.isnan(val_mae)):
        plt.loglog(val_epochs, val_mae, '--', color='#2A9D8F', label=r'$\vec{f}_{\text{MAE}}^{\text{V}}$')
    if any(~np.isnan(val_rmse)):
        plt.loglog(val_epochs, val_rmse, '--', color='#E76F51', label=r'$\vec{f}_{\text{RMSE}}^{\text{V}}$')
    
    # Add learning rate if available
    if lr_data:
        lr_agg = aggregate_by_epoch(lr_data)
        lr_epochs = [d['epoch'] for d in lr_agg]
        lr_values = [d['lr'] * 1e-3 for d in lr_agg]
        plt.loglog(lr_epochs, lr_values, '-', color='#E9C46A', label=r'$LR \times 10^{-3}$')
